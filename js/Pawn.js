...
export default class Pawn extends Piece {
    ...
    findAttacks(tiles) {
        ...
        if (this.x - 1 >= 0) {
            ...
            if (diagonalLeft && diagonalLeft.color !== this.color) {
        ...
        if (this.x + 1 < 8) {
            ...
            if (diagonalRight && diagonalRight.color !== this.color) {
                attacks.push({ x: this.x + 1, y: this.y + this.direction });
            }
        }
        return attacks;
    ...
    checkPromotion(y, tiles, x) {
        if (this.color == COLOR.BLACK && y == 7) {
            tiles[x][y] = new Queen(x, y, COLOR.BLACK, '♛');
        } else if (this.color === COLOR.WHITE && y === 0) {
            tiles[x][y] = new Queen(x, y, COLOR.WHITE, '♕');
        }
    }
}